...
module.exports = function(app,con){
  ...
  app.get('/get/sowsemen/All',(req,res) => {
    var sql = "SELECT * FROM sowsemen INNER JOIN sow ON sow.sowID = sowsemen.sowID WHERE sowsemen.isDel = 0";
  ...
  app.get('/get/sowsemen/ID',(req,res) => {
    ...
    var sql = "SELECT * FROM sowsemen INNER JOIN sow ON sow.sowID = sowsemen.sowID WHERE sowSemenID='"+sowSemenID+"' and sowsemen.isDel = 0";
    con.query(sql,function(err,result,field){
      if(err) throw err;
      var data = JSON.stringify(result);
      res.send(data);
  ...
  app.get('/getID/sowsemen/barcode',(req,res) => {
    ...
    var sql = "SELECT sowSemenID FROM sowsemen INNER JOIN sow ON sow.sowID = sowsemen.sowID WHERE barcode='"+barcode+"' and sowsemen.isDel = 0";
    con.query(sql,function(err,result,field){
      if(err) throw err;
      var data = JSON.stringify(result);
      if(data[0]){
  ...
  app.get('/get/sowsemen/barcode',(req,res) => {
    var barcode = req.query.id;
    var sql = "SELECT * FROM sowsemen INNER JOIN sow ON sow.sowID = sowsemen.sowID WHERE barcode='"+barcode+"' and sowsemen.isDel = 0";
    con.query(sql,function(err,result,field){
      if(err) throw err;
      var data = JSON.stringify(result);
      res.send(data);
